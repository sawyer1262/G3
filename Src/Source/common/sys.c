...
void NVIC_DeInit(UINT8 NVIC_PreemptionPriority, UINT8 NVIC_SubPriority, UINT8 NVIC_Channel, UINT8 NVIC_Group)
{
    UINT32 temp;
    NVIC_PriorityGroupConfig(NVIC_Group); //设置分组
    temp = NVIC_PreemptionPriority << (3 - NVIC_Group);
    temp |= NVIC_SubPriority & (0x07 >> NVIC_Group);
    temp &= 0x7;                                             //取低三位
    NVIC->ICER[NVIC_Channel / 32] |= 1 << NVIC_Channel % 32; //使能中断位(要清除的话,设置ICER对应位为1即可)
    NVIC->IP[NVIC_Channel] |= temp << 5;                     //设置响应优先级和抢断优先级
}
////外部中断配置函数
